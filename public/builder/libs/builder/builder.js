				...
				for (i in components)
				{
					...
					if (component)
					{
						...
						if (component.image) {
							...
							item.css({
								backgroundImage: "url(" +site_url+ '/builder/libs/builder/' + component.image + ")",
				...
				for (i in blocks)
				{
					...
					if (block)
					{
						...
						if (block.image) {
							...
							item.css({
								backgroundImage: "url(" + ((block.image.indexOf('//') == -1) ? 'builder/libs/builder/':'') + block.image + ")",
								backgroundRepeat: "no-repeat"
							})
						}
...
/* iframe */
    ...
    _frameLoaded : function() {
		...
		self.frameHead.append('<link data-vvveb-helpers href="' + site_url + '/builder/css/vvvebjs-editor-helpers.css?v=0.7" rel="stylesheet">');
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) 
				{
					...
					<label for="id' + j + '" style="background-image:url('+site_url+'/builder/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
