...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="row ">
            <div class="col-sm-12">
                <form class="float-xl-right float-lg-right" method="GET" action="{{ route('post.show') }}">
                    @method('GET')
                    <div class="form-row align-items-center">
                        ...
                        <div class="col-auto">
                            <select name="category" class="custom-select input-sm mb-2">
                        ...
                        <div class="col-auto">
                            <select name="author" class="custom-select input-sm mb-2">
                                <option value="">Semua Penulis</option>
                                @foreach($users as $key=>$user)
                                    <option value="{{ $user->id }}" @if(request()->get('author') == $user->id) selected @endif>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-auto">
                ...
                <form class="exe" method="POST" action="{{ route('post.deletemass') }}">
                    @csrf
                    @method('POST')
                    <div class="form-row align-items-center">
                        <div class="col-auto">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('post.destroy', $list->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a class="btn btn-success btn-sm" href="{{ route('post.edit',$list->id) }}">Edit</a>
