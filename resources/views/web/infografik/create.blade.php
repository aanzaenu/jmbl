...
<html lang="en">
  <head>
    ...
    <base href="">
    <title>Tambah Infografik - {{ env('APP_NAME') }}</title>
...
<div class="modal fade modalpertama" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" >
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<form enctype="multipart/form-data" method="POST" action="{{ route('infografik.store') }}">
				@csrf
				@method('POST')
				<div class="modal-header">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="title" name="title" placeholder="Judul Infografik" value="{{ old('title') }}">
					</div>
					<div class="form-group">
						<label for="keyword">Keyword</label>
						<input type="text" class="form-control" id="keyword" name="keyword" placeholder="Keyword Infografik" value="{{ old('keyword') }}">
					</div>
					<div class="form-group">
						<label for="status">Status</label>
						<select name="status" class="custom-select">
							<option value="0" @if(old('description') == 0) selected @endif>Draft</option>
							<option value="1" @if(old('description') == 1) selected @endif>Publish</option>
						</select>
					</div>
					<div class="form-group">
						<label for="tamnel">Thumbnail</label>
						<div class="custom-file">
							<input name="tamnel" type="file" class="custom-file-input" id="thumbnail" value="{{ old('tamnel') }}">
							<label class="custom-file-label" for="thumbnail">Pilih Gambar</label>
						</div>
					</div>
					<div class="form-group">
						<label for="description">Deskripsi</label>
						<textarea name="description" class="form-control" id="description" rows="2" placeholder="Deskripsi Infografik">{{ old('description') }}</textarea>
						<textarea name="content" class="d-none konten">{!! old('content') !!}</textarea>
					</div>			
				</div>
				<div class="modal-footer">
...
$(document).ready(function() 
{
	...
	Vvveb.Builder.init("about:blank", function() {
		...
		@if(!empty(old('content')))
			// Vvveb.Builder.setHtml("<h1>oke</h1>");
		@endif
	});
