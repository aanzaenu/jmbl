    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('userlist.update', $row->id) }}">
                            @csrf
                            @method('PATCH')
                            <div class="form-group">
                                <label for="name">Nama</label>
                                <input type="text" class="form-control @if($errors->has('name')) is-invalid @endif" id="name"  name="name"  placeholder="Nama" value="{{ $row->name }}">
                            </div>
                            <div class="form-group">
                                <label for="email">Email</label>
                                <input type="email" class="form-control @if($errors->has('email')) is-invalid @endif" id="email"  name="email"  placeholder="Email" value="{{ $row->email }}">
                            </div>
                            <div class="form-group">
                                <label for="group">Role</label>
                                <select class="custom-select @if($errors->has('group')) is-invalid @endif" name="group">
                                    @foreach($groups as $key=>$group)
                                        <option value="{{ $group->id }}" @if($row->usergroup->group_id == $group->id) selected @endif>{{ $group->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
